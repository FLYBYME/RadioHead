...
RH_RF22::RH_RF22(uint8_t slaveSelectPin, uint8_t interruptPin, SPIClass& spi)
	:
...
{
	if (!RHSPIDriver::init()) {
		Serial.println("RHSPIDriver::init fail");
		return false;
	}
...
void RH_RF22::handleInterrupt()
{
	...
	if (_lastInterruptFlags[0] & RH_RF22_IPKVALID)
	{
		...
		spiBurstRead(RH_RF22_REG_7F_FIFO_ACCESS, _buf, len);
...
bool RH_RF22::send(const uint8_t* data, uint8_t len)
{
	...
	spiBurstWrite(RH_RF22_REG_7F_FIFO_ACCESS, data, len);

	spiWrite(RH_RF22_REG_3E_PACKET_LENGTH, len); // Total length that will be sent
	setModeTx();
